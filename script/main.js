      ...
      dataArr.map(customData => {
        ...
        if (dataArr.length === dataArr.indexOf(customData) + 1) {
          ...
          if (/Mobi|Android/i.test(navigator.userAgent)) {
            ...
            const orientationWrapper = document.querySelector(".orientation-wrapper");
            ...
            // Wait 2 seconds after fade in, then fade out
            setTimeout(() => {
              overlay.style.opacity = 0;
            }, 2100);

            // After fade out completes, show content & start animation
            setTimeout(() => {
              overlay.style.display = "none";
              orientationWrapper.style.display = "block";
              orientationWrapper.classList.add("start-animation");
              animationTimeline();
            }, 3200);
...
const animationTimeline = () => {
  ...
  tl
    ...
    .staggerFromTo(".baloons img", 2.5, { opacity: 0.9, y: 1400 }, {
      opacity: 1, y: -1000
    }, 0.2)
    .from(".lydia-dp", 0.5, {
